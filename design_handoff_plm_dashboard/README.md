...
### User
`id` · `name` · `initials` · `role` · `title` · `color` · `capacityPerWeek`(⚠ OP에 원천 없음 — 외부 설정) · `isGroup`(boolean, `_type==="Group"`) · `isObserver`(boolean, role="Observer") · `isBot`(boolean, name/login이 "form-reporter" 패턴)

> ⚠ `isGroup`/`isObserver`/`isBot` 항목은 `D.U` lookup에서 제거하지 말 것. 담당자 WP/프로젝트 leadId가 해당 ID를 참조할 수 있다. UI 드롭다운·userUtilization()에서만 `.filter(u => !u.isGroup && !u.isObserver && !u.isBot)`로 제외한다.
...
9. **Live data 누락 필드** — op-adapter.js가 project에서 `nameKo`·`startDate`·`dueDate`를 아직 매핑하지 않음 → Projects 헤더에 일부 undefined 표시 가능 (이슈 #11 추적 중).
...
- **과제 선택/숨김:** Overview 상단 chip bar에서 과제 클릭 → 전체 뷰(Board/Timeline/Projects) 연동. `state.hiddenProjects`로 localStorage 유지. Projects 뷰 subtab × 버튼으로도 숨김 가능
