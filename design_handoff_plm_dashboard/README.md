...
16. **Resources Load는 Estimate/Time entry 입력률 없이는 보조 지표다** — 운영 OP 기준 Open WP 196건 중 Estimated time 입력 4건(2%), Time entries 0건이다. 이 상태에서 AVG LOAD/OVERLOADED를 전면에 두면 모든 인력이 여유로워 보이는 착시가 생긴다. Resources는 Assignment/Estimate/Due Date 커버리지와 Missing Assignee/Estimate/Due 입력 유도를 먼저 보여주고, Capacity Signal은 `Load confidence Low`로 표시해야 한다 (#48, AC-UX-19).
