...
**증상:** Resources가 AVG LOAD/OVERLOADED/UNDERUTIL 중심으로 보였지만 운영 OP는 Open WP 194건 중
...
## 2026-06-20 — 의도적 제외 프로젝트가 대시보드 집계에 포함됨 (#50)

**증상:** 운영 의도상 대시보드에서 제외해야 하는 `인프라 구축` 프로젝트가 `DB.PROJECTS`에 남아
Projects/Board/Timeline/Resources 집계 대상이 됐다. 이 프로젝트에는 bot 계정도 프로젝트 역할에
남아 있어 역할 검증이 실제 운영 의도와 어긋났다.

**근본 원인:** 기존 전역 제외 정책이 `DR 사업본부 주관 미팅` 유형에만 적용됐고, `인프라 구축`은
예외적으로 bot 필터를 우회했다. 뷰 단위 숨김이나 특정 화면 필터만으로는 다른 창 집계에 계속
남을 수 있으므로, 프로젝트 대상 정책은 OP 어댑터의 allowed project set에서 결정해야 한다.

**수정 내용:**
- `op-adapter.js`: `인프라 구축`을 `allowedProjectIds` 생성 단계에서 제외해 `PROJECTS`와 `WORK_PACKAGES`에 진입하지 않게 처리
- `op-adapter.js`: 인프라 전용 bot 역할 예외 제거, 모든 프로젝트에 동일한 bot 필터 적용
- `e2e-role-check.py`: 과거 PM/PL 기대값을 실제 계약인 PM/TL로 보정하고 `AC-ROLE-01b`로 인프라 제외 검증 추가

**검증:**
- `python3 proxy/e2e-role-check.py`: `DB.PROJECTS 26개`, `인프라 구축` 제외, bot/observer role 0건, 11/11 통과
- `python3 proxy/e2e-verify.py`: 25/25 통과, `PROJECTS.length=26`
- `python3 proxy/e2e-verify-v2.py`: 17 pass / 0 fail / 3 expected warnings
- Playwright 직접 조회: Projects/Board/Timeline/Resources/Overview/Risks `#content`에 `인프라 구축` 문자열 0건

**Anti-pattern:**
> ❌ WRONG: 운영 제외 프로젝트를 특정 뷰에서만 숨기거나 bot 예외로 보정
> ✅ CORRECT: `op-adapter.js`에서 allowed project set을 먼저 만들고 모든 downstream collection을 같은 set으로 필터링
...
- [ ] 운영 의도상 제외한 프로젝트는 `op-adapter.js`의 DB 진입 전 필터에 포함되는가?
