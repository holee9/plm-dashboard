...
### User
`id` · `name` · `initials` · `role` · `title` · `color` · `capacityPerWeek`(⚠ OP에 원천 없음 — 외부 설정) · `isGroup`(boolean, `_type==="Group"`) · `isObserver`(boolean, role="Observer")

> ⚠ `isGroup`/`isObserver` 항목은 `D.U` lookup에서 제거하지 말 것. 담당자 WP/프로젝트 leadId가 해당 ID를 참조할 수 있다. UI 드롭다운에서만 `.filter(u => !u.isGroup && !u.isObserver)`로 제외한다.
...
6. **D.U lookup 안전 규칙** — `D.U[id]` 직접 `.property` 접근 금지. 반드시 `D.U[id]?.property ?? fallback` 형태로 사용. USERS 배열 필터링 시 D.U도 같이 비워지므로 컬렉션 자체 제거 대신 플래그(`isGroup`, `isObserver`) 방식을 사용할 것.
7. **Live data 누락 필드** — op-adapter.js가 project에서 `nameKo`·`startDate`·`dueDate`를 매핑하지 않음 → Projects 헤더에 `undefined` 표시 가능. 추후 op-adapter 보완 필요(이슈 #12).
