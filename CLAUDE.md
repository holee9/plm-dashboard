...
## 14. Issue-Based Work Tracking

All work must be tracked through GitHub Issues for full history management.
For detailed procedures, see @.claude/rules/moai/workflow/issue-tracking.md

### HARD Rules (Mandatory)

- [HARD] Register Every Work Plan as an Issue: Before starting any work plan, create a GitHub issue (`gh issue create`) that captures the goal, scope, and acceptance criteria
  WHY: Every plan must leave a traceable history record from its inception

- [HARD] Log Progress as Issue Comments: While working on an issue, record meaningful progress, decisions, and results as issue comments (`gh issue comment`)
  WHY: Work history must be reconstructable from the issue timeline alone

- [HARD] Register Discovered Work as New Issues: Any additional work discovered during execution MUST be filed as its own issue and linked to the originating issue
  WHY: No follow-up work may be lost or tracked only in memory

### Execution Notes

- Repository: `holee9/plm-dashboard` (Issues enabled)
- Reference the issue number in related commits (e.g., `#12`) for cross-linking
- When delegating to subagents, pass the active issue number so they comment on the same issue
