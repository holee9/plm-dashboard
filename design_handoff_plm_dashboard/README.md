...
> 확인 대상은 `http://127.0.0.1:8080/PLM%20Dashboard.html`의 로컬 라이브 서비스입니다.
> 운영 프록시의 API를 사용하며, 원격 서버의 정적 파일 배포 버전까지 검증한 것은 아닙니다.
...
다음은 기존 화면 배치가 핸드오프와 다르다는 판정과 구분한다. 현재 동작을 개선 목표로 승인한 것은 아니다.

- [#59](https://github.com/holee9/plm-dashboard/issues/59): Overview 완료율의 고정 증감 `▲ 4%` 제거/실계산.
- [#61](https://github.com/holee9/plm-dashboard/issues/61): `Last 90d`는 현재 `data-noop` 버튼이며 전역 기간 필터가 아니다.
- [#62](https://github.com/holee9/plm-dashboard/issues/62): Board 컬럼 30건, Timeline WP 22건(유효한 기간 우선), Risks 마감일 미설정 80건의 표시 제한 이후 탐색.
- [#63](https://github.com/holee9/plm-dashboard/issues/63), [#64](https://github.com/holee9/plm-dashboard/issues/64): 키보드·아이콘 접근성과 모바일 툴바.
- [#65](https://github.com/holee9/plm-dashboard/issues/65), [#66](https://github.com/holee9/plm-dashboard/issues/66): 산출 불가와 0% 구분, 중복 라벨·빈 분석 패널·가독성.
...
드래그 순서의 저장/복원, PM/TL 복수 선택, 원격 정적 배포 파일은 이번 검증 범위에 포함하지 않았다.
