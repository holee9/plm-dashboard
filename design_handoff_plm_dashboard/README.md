...
대상 규모: 대시보드 대상 프로젝트 26개 (DR 사업본부 주관 미팅 유형과 `인프라 구축` 제외),
개발자 11명, WP 374건.
...
### Relation
`id` · `type`(`follows` 등) · `fromId` · `toId` · `delay`. 현재 운영 OP는 relation 28건 중 일정 선후관계 `follows`가 3건뿐이므로, Timeline은 critical path를 계산하지 않고 Schedule Inspection에서 의존성 참고 신호로만 표시한다.
...
| 항목 | 계산 기준 | 표시 위치 |
|---|---|---|
| `Coverage` | Open non-milestone WP 중 `_start`와 `_due`가 모두 있는 비율 | Schedule Inspection metric |
...
운영 OP 기준 Estimated time 입력률이 2%, Time entries가 0건이므로 기존 AVG LOAD/OVERLOADED 중심
화면은 업무량을 과소 표시한다.
...
16. **Resources Load는 Estimate/Time entry 입력률 없이는 보조 지표다** — 운영 OP 기준 Open WP 194건 중 Estimated time 입력 4건(2%), Time entries 0건이다. 이 상태에서 AVG LOAD/OVERLOADED를 전면에 두면 모든 인력이 여유로워 보이는 착시가 생긴다. Resources는 Assignment/Estimate/Due Date 커버리지와 Missing Assignee/Estimate/Due 입력 유도를 먼저 보여주고, Capacity Signal은 `Load confidence Low`로 표시해야 한다 (#48, AC-UX-19).
...
- **Resources 레이아웃:** Projects와 같은 황금비 근사 `8fr / 5fr` 주/보조 그리드. 좌측은 Data Readiness + Person Pressure, 우측은 Capacity Signal + Input Actions. Desktop 1440px 실측 기준 컬럼 721:451=1.600, 상단/하단 행 높이 오차 0px. 모바일은 단일 컬럼으로 접고 정렬 버튼·표는 내부 스크롤 처리한다 (#48, AC-UX-19).
